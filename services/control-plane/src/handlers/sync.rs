...
pub async fn get_bot_config(
    ...
    state
        .metrics
        .increment(metrics::CONFIG_FETCH_COUNT, 1)
        .await;
...
pub async fn report_wallet(
    ...
    let decoded = bs58::decode(&req.wallet_address).into_vec().map_err(|_| {
        (
            StatusCode::BAD_REQUEST,
            "Invalid Solana wallet address: not valid Base58".to_string(),
        )
    })?;
    ...
    let result = sqlx::query("UPDATE bots SET agent_wallet = $1, updated_at = NOW() WHERE id = $2")
    ...
    match result {
        ...
        Ok(_) => {
            info!(
                "Bot {} reported wallet address: {}",
                bot_id, req.wallet_address
            );
            state
                .metrics
                .increment(metrics::WALLET_REPORT_COUNT, 1)
                .await;
            Ok(StatusCode::OK)
        }
        Err(e) => {
            ...
            state
                .metrics
                .increment(metrics::WALLET_REPORT_ERRORS, 1)
                .await;
...
pub async fn heartbeat(
    ...
    sqlx::query("UPDATE bots SET last_heartbeat_at = NOW(), updated_at = NOW() WHERE id = $1")
        .bind(bot_id)
        .execute(&state.db)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    if let Some(metrics_batch) = req.metrics {
        ...
        state
            .metrics
            .increment(metrics::METRICS_BATCH_RECEIVED, batch_len as u64)
            .await;
    }
...
pub async fn ingest_events(
    ...
    state
        .metrics
        .increment(metrics::EVENTS_INGESTED, event_count)
        .await;
    state
        .metrics
        .increment(metrics::EVENTS_TRADES, trade_count)
        .await;
    if error_count > 0 {
        state
            .metrics
            .increment(metrics::EVENTS_ERRORS, error_count)
            .await;
    }
    ...
    Logger::bot_event(
        ...
        &format!(
            "count={}, trades={}, errors={}",
            event_count, trade_count, error_count
        ),
    );
...
pub async fn get_bot_secrets(
    ...
    let stored_token = bot.bootstrap_token.as_ref().ok_or_else(|| {
        (
            ...
            "No bootstrap token configured for this bot".to_string(),
        )
    })?;
    ...
    if bot.bootstrap_token_used_at.is_some() {
        ...
        return Err((
            StatusCode::FORBIDDEN,
            "Bootstrap token already used".to_string(),
        ));
    }
    ...
    if rows_affected == 0 {
        ...
        return Err((
            StatusCode::FORBIDDEN,
            "Bootstrap token already used (concurrent)".to_string(),
        ));
    }
    ...
    let jupiter_api_key =
        config::get_config_decrypted_or(&state.db, &state.secrets, keys::JUPITER_API_KEY, "").await;
